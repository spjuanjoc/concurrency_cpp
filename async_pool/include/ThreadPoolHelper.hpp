...
public:
  ...
  void waitAndPop(T& value)
  {
    ...
    data_condition_v_.wait(lock, [this] { return !data_queue_.empty(); });
...
class FunctionWrapper
{
  ...
  {
    F f;
    explicit ImplType(F&& func) : f(std::move(func)) {}
    void call() override { f(); }
  };

public:
  template<typename F>
  FunctionWrapper(F&& f) : impl_(new ImplType<F>(std::forward<F>(f))) //! No explicit
  {
  }
